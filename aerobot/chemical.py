...
def get_rna_features(nt1_df:pd.DataFrame) -> pd.Series:
    ...
    :param nt1_df: A DataFrame containing the counts of single nucleotides, either RNA or DNA. Assumed to single stranded.
    :return: A pd.Series containing the formal C oxidation state of the RNA coding sequences.
    '''
    my_nts = sorted(set(nt1_df.columns).intersection(CANONICAL_NTS_ALL))
    canonical_data = nt1_df[my_nts]
...
def get_aa_features(aa1_df:pd.DataFrame) -> pd.Series:
    ...
    :param aa1_df: A DataFrame containing the counts of single amino acids.
    :return: A pd.Series containing the formal C oxidation state.
    '''
    aas = sorted(set(aa1_df.columns).intersection(CANONICAL_AAS))
    canonical_data = aa1_df[aas]
...
def chemical_get_features(metadata_df:pd.DataFrame=None, cds1_df:pd.DataFrame=None, aa1_df:pd.DataFrame=None, nt1_df=None) -> pd.DataFrame:
    ...
    :param nt1_df: DataFrame containing the nt_1mer feature data.
    :param aa1_df: DataFrame containing the aa_1mer feature data.
    :param cds1_df: DataFrame containing the cds_1mer feature data.
    :return: A DataFrame containing the chemical feature data.
    '''
    n_genes = metadata_df.drop_duplicates()['number_of_genes']
    gc_content = get_gc_content(nt1_df)
    aa_features = get_aa_features(aa1_df)
    rna_features = get_rna_features(cds1_df)
